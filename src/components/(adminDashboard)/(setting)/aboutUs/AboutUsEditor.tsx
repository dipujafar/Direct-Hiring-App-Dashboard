...
} from "@/redux/api/patApi";
import { useEffect } from "react";
import { toast } from "sonner";
import { getErrorMessage } from "@/utils/getErrorMessage";

const AboutUsEditor = () => {
  const [updatePrivacy, { isLoading }] = useUpdatedAboutUsMutation();
  const { data } = useGetAboutUsQuery(undefined);
  const privacyPolicy = data?.data;
  ...
  const editor = useEditor({
    ...
    content: "<p>Start writing your about us policy...</p>",
    editorProps: {
      attributes: {
        class:
  ...
  useEffect(() => {
    if (editor && privacyPolicy?.description) {
      editor.commands.setContent(privacyPolicy?.description);
    }
  }, [editor, privacyPolicy]);
  ...
  const saveTerms = async () => {
    ...
    try {
      ...
      const response = await updatePrivacy(dataInfo).unwrap();
      if (response.success) {
        toast.success(
          response.message || "Successfully updated about us policy"
        );
      }
    } catch (error) {
      console.log("error_____", error);
      toast.error(getErrorMessage(error) || "Failed to update about us policy");
    }
  };

  return (
    <div className="space-y-4 border border-ns-stroke p-4 rounded-lg">
      <h2 className="text-2xl font-semibold text-ns-title">About Us Editor</h2>
